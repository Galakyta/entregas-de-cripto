...
def texto_para_blocos(texto):
    dados = texto.encode() #encoda por padrao pra utf 8, mas tem outras formas de encodar como ascii
...
def encrypt_cbc(plaintext_bytes, w, iv):
    blocos = texto_para_blocos(plaintext_bytes) if isinstance(plaintext_bytes, str) else [plaintext_bytes]
    anterior = iv
    resultado = b''
    for bloco in blocos:
...
def derivar_chave(senha, salt = None): # ok pontos importantes aqui, o salt é só um valor aleatorio que você mistura na senha pra ela ficar diferentes na derivação
    # mesmo que duas senhas sejam iguais, ent se eu  fizer a senha makonhilda123 e o outra pessoa tmbm, elas acabam diferentes pelo salt, a nao ser que o satl seja o mesmo mai ai
    # tmbm n tem mt como eu te ajudar ne parça
    if salt is None: #
        salt = os.urandom(16) # esse if aq é pra implementacao do cofre, onde isso deixa a gente reutilizar o salt
         #nn sei se era pra usar essa lib de os, mas oh well usei se meu crime é amar de mais me prendam
    #preciso descobrir qq é  um ataque de dicionario e rainbow tables
    chave = hashlib.pbkdf2_hmac( # isso aq eu ainda preciso implementar na mão mas vou deixar pra depois
        'sha256', # dai tem o self hard 256 q eu to morrendo de preguiça de aprender mas é o alg de hashing
        senha.encode(), # joga a senha em bytes
        salt,
        100000, # e o numero de iterações
        dklen=32  # AES-256 isso aqw da uma merda se tu botar a de 16 pq vc e uma anta e fez 128 e adaptou depois boa marcos
    )
    return chave, salt

#

# nucleo da parada que eu nn aguento mais ver na minha frente:
